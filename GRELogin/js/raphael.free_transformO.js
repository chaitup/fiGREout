	...
	Raphael.fn.freeTransform = function(subject, options, callback) {
		...
		ft.updateHandles = function() {
			...
			if ( ft.handles.center ) {
				ft.handles.center.disc.toFront().attr({
					cx: ft.attrs.center.x + ft.attrs.translate.x - 60,
					cy: ft.attrs.center.y + ft.attrs.translate.y - 10
				});
			}
		...
		ft.showHandles = function() {
			...
			ft.axes.map(function(axis) {
				...
				ft.handles[axis].disc = paper                     //ft.opts.size.axes
					.circle(ft.attrs.center.x, ft.attrs.center.y, 0)
					.attr(ft.opts.attrs)
					;
			});
			...
			if ( ft.opts.draw.indexOf('bbox') >= 0 ) {
				...
				var i, handle;
// 4 : 8 => 4:4
				for ( i = ( ft.opts.scale.indexOf('bboxCorners') >= 0 ? 0 : 4 ); i < ( ft.opts.scale.indexOf('bboxSides') === -1 ? 4 : 4 ); i ++ ) {
					handle = {};
